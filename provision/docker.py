...
def build(
    context,
    env="development",
    builder="public.ecr.aws/saritasa/buildpacks/google/builder:12.22",
    runner="public.ecr.aws/saritasa/buildpacks/google/runner:12.22",
    tag="ffgpt-backend",
):
...
def up_containers(
    ...
    **kwargs,
    ...
    up_cmd = (
        f"docker compose up "
        f"{'-d ' if detach else ''}" f"{' '.join(containers)}"
    )
    try:
        context.run(up_cmd)
